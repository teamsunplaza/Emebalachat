...
// Frame write on the orchestrator side (one message, synchronous completion —
// the orchestrator half of the M-2 flush guarantee).
bool WritePipeFrame(HANDLE pipe, std::string_view json) {
...
bool WorkerManager::SendToWorker(const std::wstring& family, std::string_view json) {
    ...
    const bool ok = WritePipeFrame(w->pipe, json);
    if (!ok) {
...
// pipe. The manager mutex serializes this against SendToWorker / GracefulStop
// (both touch the same HANDLE), which is REQUIRED for a duplex pipe with one
// server handle: overlapped reads and writes may interleave, but handle
// lifecycle must not.
WorkerManager::WorkerRead WorkerManager::ReadFromWorker(const std::wstring& family,
    ...
    switch (ReadPipeFrame(w->pipe, json, static_cast<DWORD>(timeout_ms))) {
...
bool WorkerManager::GracefulStop(WorkerHandle& w, int timeout_ms) {
    ...
    const bool sent = WritePipeFrame(w.pipe, wp::BuildShutdown());
    if (!sent) {
